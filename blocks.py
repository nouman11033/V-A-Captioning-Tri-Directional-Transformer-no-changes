...
# numaan the code you have here is wrong fam 
#It looks like you're trying to define a neural network module using PyTorch called LayerStack. This module is designed to stack multiple instances of a given layer and pass input through each of these layers sequentially. However, I noticed a couple of issues in your code:
#clone function: The clone function is not defined in your code. Instead, you should use deepcopy to create copies of the layer. You can import it using from copy import deepcopy.
#Layer initialization: You need to ensure that you're actually creating instances of the layer you pass to the LayerStack constructor. If the layer is a PyTorch module, you should call it inside the loop to create separate instances.
...
#            x = layer(x, masks)
#        return x
#Used nn.ModuleList to store the layers instead of a regular Python list. This is important for proper tracking and management of the layers by PyTorch.
#Inside the constructor, I create separate instances of the layer using deepcopy and store them in self.layers.
#This corrected code should work for creating a stack of layers and passing inputs through them. Just make sure that the layer you pass to LayerStack is a PyTorch module with a valid forward method.
...
#boy, the forward method is used to define the forward pass of the neural network, where input data is passed through the network's layers for computation.
    def forward(self, x, masks):
        for layer in self.layers:
            x = layer(x, masks)
        return x
